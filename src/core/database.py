...
class QdrantRepository:
    ...
    def search(
        ...
        """
        Ищет похожие векторы в базе данных.

        Args:
            query_vector: Вектор запроса (эмбеддинг).
            limit: Максимальное количество результатов.
            score_threshold: Минимальный порог схожести (отсеивает мусор).
            query_filter: Фильтр по метаданным (payload).
        ...
        try:
            results = self.client.search(
                ...
                with_payload=True,
                with_vectors=False,  # Экономим трафик, векторы в ответе обычно не нужны
            )

            # Форматируем вывод для удобства использования в бизнес-логике
            formatted_results = [
                {
                    "id": hit.id,
            ...
            logger.info(
                f"🔍 Поиск в '{target_collection}': найдено {len(formatted_results)} результатов (limit={limit})")
    ...
    def delete_by_ids(
        ...
        """
        Удаляет записи по списку их ID.
        ...
        try:
            ...
            logger.info(f"🗑️ Успешно удалено {len(ids)} записей из '{target_collection}'")
            return True

        except Exception as e:
            logger.error(f"❌ Ошибка удаления записей: {e}")
            return False
    ...
    def delete_by_filter(
        ...
        """
        Удаляет записи, соответствующие фильтру (например, все новости старого кластера).
        """
        target_collection = collection_name or self._collection_name
        try:
            self.client.delete(
            ...
            logger.info(f"🗑️ Успешно удалены записи по фильтру из '{target_collection}'")
            return True
        except Exception as e:
